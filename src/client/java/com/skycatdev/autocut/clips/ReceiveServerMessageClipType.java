...
public class ReceiveServerMessageClipType extends ClipType {
    ...
    public Clip createClip(long time, Text message, boolean actionBar) { // TODO: Show what server you are connected to, make a place for actionBar to go
        return new Clip(time - getStartOffset(), time, time + getEndOffset(), ID, isActive(), isInverse(), getExportGroupingMode(), "Received a chat message from the server" + (actionBar ? " in action bar" : ""), "server", message.getString(), null, null);
    }
