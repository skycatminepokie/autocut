...
public class UseItemClipType extends ClipType {
    ...
    public Clip createClip(long time, PlayerEntity player, ItemStack itemStack) {
        return new Clip(time - getStartOffset(), time, time + getEndOffset(), ID, isActive(), isInverse(), getExportGroupingMode(), "Used " + itemStack.getName().getString(), player.getNameForScoreboard(), Registries.ITEM.getId(itemStack.getItem()).toString(), player.getPos(), null);
    }
}
