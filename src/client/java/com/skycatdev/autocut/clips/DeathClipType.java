...
public class DeathClipType extends ClipType {
    ...
    public DeathClipType(boolean active, boolean shouldRecord, long startOffset, long endOffset) {
    ...
    public Clip createClip(long time, ClientPlayerEntity player, Text deathMessage) {
        return new Clip(time - getStartOffset(), time, time + getEndOffset(), ID, isActive(), deathMessage.getString(), null, player.getNameForScoreboard(), null, player.getPos());
    }
}
